/*
 *  Milad Milad miladim@cs.uni-freiburg.de
 * */

/** \file
*** \brief mmfold program source code, adapted from Vienna RNAfold source code
***
*** This code provides an interface for mm Partition function folding
...
#include "mmfold_cmdl.h"



/*@unused@*/
static char UNUSED rcsid[] = "$Id: mmfold.c,v 0.1 2016/0/24 19:51:00 milad Exp $";
...
int main(int argc, char *argv[]){
  FILE            *input, *output;
  struct          mmfold_args_info args_info;
  ...
  #############################################
  */
  if(mmfold_cmdline_parser (argc, argv, &args_info) != 0) exit(1);
  /* temperature */
  ...
  if(args_info.partfunc_given){
	printf ("Partition function given!\n");
    pf = 1;
//    if(args_info.partfunc_arg != 1) // TODO: this if ignored, check it later
    do_backtrack_mm = 0;
  ...
  mmfold_cmdline_parser_free (&args_info);
...
//          pl1     = vrna_plist_from_probs(vc, bppmThreshold);
          pl1     = vrna_plist_from_probs_mm(vc, bppmThreshold);
          ...
          if (fname[0]!='\0') {
            strcpy(ffname, fname);
            strcat(ffname, "_mm_dp.ps");
          } else strcpy(ffname, "mm_dot.ps");
